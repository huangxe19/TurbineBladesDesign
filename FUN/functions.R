...
# objective function
eval_f <- function(x, gpi_stress, Xref, p_d){
  return(- sqrt(alcGPsep(gpi_stress, matrix(x, nrow=1), Xref)))
}

# constraint
eval_g <- function(x, gpi_stress, Xref,p_d) {
  return(1.3e-3 - p_d$mean)
}



# optimize ALC objective under constraint
optim_alc <-function(x_start, eval_f, eval_g, opts, gpi_stress, xref, pos_disp) {
...
ALC <- function(X_raw, y, XX_raw, yy, niter) {
  ...
  mle_disp <- mleGPsep(gpi_disp , param='d' , tmin=d_disp$min, tmax=d_disp$max, ab=d_disp$ab)
  p_disp <- predGPsep(gpi_disp, XX_raw, lite=TRUE)
  ...
  mle_disp <- rbind(mle_disp, 
                    mleGPsep(gpi_disp , param='d' , tmin=d_disp$min, tmax=d_disp$max, ab=d_disp$ab))
  ...
  return(list(X = X, y = cbind(y_s, y_d), rmse.alc = rmse.alc, mle = mle))
}
